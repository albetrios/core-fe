...
- **Stale chunks are the second path** `src/core/version/stale-chunk-recovery.ts`: a tab open across a deploy still `import()`s the old hashed chunks. `netlify.toml` answers a missing `/assets/*` with a real **404** (never the SPA `index.html` rewrite, whose 200 surfaces as a bogus MIME-type error), so `vite:preloadError` fires and the same `reloadOntoLatestBuild()` runs — once per `buildId`, under its OWN sessionStorage key, and never over a focused field. Both reload paths share `reload-marker.ts` and `lib/editable-focus.ts`; `pnpm build:check` also tripwires any source map that would reach `dist/`.
