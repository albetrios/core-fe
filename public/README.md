...
| **locale-init.js**       | Prevents locale/dir FOUC: reads `locale-preference` and sets `lang`/`dir` before React.            | Update if locale store key or RTL set changes (see `useLocaleStore` / `RTL_LOCALES`).       |
