...
| `my-organization-summaries.ts` (+ .test) | The caller's organization list under ONE cache key, `myOrganizationsQueryKey`: `fetch`/`ensure`/`useMyOrganizationSummaries` + `invalidateMyOrganizationSummaries`. The guard chain, switcher, dashboard, command palette, picker, Settings and onboarding all read it, so a create, rename or logo change refreshes every surface by invalidating it |
