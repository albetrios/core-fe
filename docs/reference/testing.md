...
| `theme-shape.e2e.test.ts`                          | "Square means square", **measured**: seeds radius None + Sharp and sweeps the computed `border-radius` of every painted element on sign-in, the dashboard (desktop + phone shell) and the open overlays — must be empty; a control proves the sweep can see corners, and the Round look must grow them |
