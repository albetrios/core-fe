...
| `sonner`                           | Yes   | Deferred `shared/notify/notify-runtime.tsx` renderer; the root-mounted `AppToaster` and `notify` bridge provide immediate feedback and stable IDs.                                                       |
...
- **Stale-chunk recovery:** `src/core/version/stale-chunk-recovery.ts` — the poll is not the only way a superseded deploy shows up. A tab open across a deploy still holds the OLD chunk graph, so its next `import()` requests a content-hashed file the CDN no longer has and Vite dispatches **`vite:preloadError`**. That event alone is not enough to act on: offline, a flaky CDN, and the cosmetic `initDeferredIconSets()` / `preloadBootRoutes()` prefetches (whose chunks are deliberately outside the SW precache) all raise it, and reloading for those would abort real work — an in-flight `/auth/refresh` on a cold load, most sharply. So the handler asks **`isNewBuildAdvertised()`** first and reloads only when `version.json` reports a different `buildId`, through the same `reloadOntoLatestBuild()`. It stands down while a field is focused (re-checked after the probe) and acts at most once per `buildId`, under its own sessionStorage key so neither reload path spends the other's attempt. The event is never `preventDefault()`ed — Vite's helper is `import().catch(dispatch)`, so suppressing the rethrow makes the import resolve with `undefined` and the caller report a misleading `TypeError` instead of the real "Failed to fetch dynamically imported module". `netlify.toml` additionally answers a missing `/assets/*` with a real **404** rather than rewriting it to `index.html` with a 200 (the opaque MIME-type error), but recovery works either way.
