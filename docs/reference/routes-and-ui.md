...
| `/accept-invite/$invitationId`              | Accept membership invite — reached from an invite email but **auth-required** (`requireAuth` in `beforeLoad`); no session → `/login` carrying this URL (`?token=` included) as `redirect`. On accept: switch to the org, refresh the session, land on its dashboard | `POST /api/v1/invitations/{id}/accept`, `POST /auth/switch-to-organization`, `POST /auth/refresh`, `GET /auth/me/context` |
