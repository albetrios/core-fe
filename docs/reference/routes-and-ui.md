...
| `/mfa`                                      | MFA (TOTP) verification                                                                                                                                                                                                                                             | `POST /auth/mfa/login`, `GET /users/me`                                                                                                      |
| `/callback/$provider`                       | OAuth callback (token exchange + redirect)                                                                                                                                                                                                                          | token exchange against auth service                                                                                                          |
...
| `/onboarding`                               | Post-signup wizard (profile → org → invite → done) → new org's dashboard                                                                                                                                                                                            | `POST /api/v1/tenancy/organizations`, invitation endpoints                                                                                   |
| `/accept-invite/$invitationId`              | Accept membership invite — reached from an invite email but **auth-required** (`requireAuth` in `beforeLoad`); no session → `/login` carrying this URL (`?token=` included) as `redirect`. On accept: switch to the org, refresh the session, land on its dashboard | `POST /api/v1/tenancy/invitations/{invitation_id}/accept`, `POST /auth/switch-to-organization`, `POST /auth/refresh`, `GET /auth/me/context` |
...
| `#settings/account/…`                       | `profile` · `account` · `security` · `notifications` · `sessions` · `billing` · `integrations` — global `SettingsModal` over any page; `integrations` needs `api-key:read`                                                                                          | profile, MFA, passkey, session, api-key endpoints                                                                                            |
| `#settings/organization/…`                  | `general` · `members` · `roles` — TEAM workspaces only; needs organization context + section permission                                                                                                                                                             | org endpoints per section                                                                                                                    |
| **System**                                  |                                                                                                                                                                                                                                                                     |                                                                                                                                              |
| `/*` (no match)                             | 404                                                                                                                                                                                                                                                                 | —                                                                                                                                            |
...
- Token refresh: `POST /auth/refresh` (cookie), `GET /users/me`.
...
Other shared pieces: **FormError**, **FormField** (forms), **FullPageSpinner**, **RetryError**, **QueryBoundary** (query loading/error wrapper), **DataTable** (and pagination/toolbar/column header), **InviteMemberDialog**, **CreateOrganizationDialog**, **OrganizationSwitcher**, **OrganizationBadges**, **SettingsModal** (global, hash-driven), **CommandPalette** (cmdk), **PermissionGuard**. Layouts: **AuthLayout** (mounted by the pathless `auth-shell` route), **AppLayout** (mounted by the `$organizationSlug` island); the root shell lives in `app/routes/routeTree.tsx`.
