...
Related: `fe-guard-authoring` (gates that actually gate) · `fe-before-commit-guard`
(commit-time gate) · `fe-project-health-check` (full audit) · `fe-change-completeness`
...
the retryability caveat live in **`fe-bundle-performance`** (its owner — do not
restate them here).
...
prevent. Owner: **`fe-test-generation`**.
...
Owner: **`fe-i18n-constants`** (copy) and **`fe-locale-formatting`** (values). Sweep
