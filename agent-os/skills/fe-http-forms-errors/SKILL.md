...
**Data fetching patterns:** `react-best-practices` + `agent-os/rules/fe-api-data-patterns.mdc`
**Resilience rules:** `agent-os/rules/fe-resilient-interactions.mdc` — single-flight writes (§1), `useAppMutation` for every write (§20), all three query states (§8), four states with the shared components (§19), dialog exits held during a write (§26)
...
- `auth-form-pending.ts`; copy that shape (stable label, icon→spinner, per-method captcha gate,
  ...
  `agent-os/rules/fe-component-patterns.mdc` → "Auth Method Buttons".
...
- Skills: `composition-patterns`, `fe-test-generation`, `fe-resource-crud`
- Components: `QueryBoundary`, `RetryError`, `OfflineIndicator`
- Rules: `agent-os/rules/fe-api-data-patterns.mdc`, `agent-os/rules/fe-resilient-interactions.mdc`
