...
- The fe-testing-requirements rule or fe-code-structure skill invokes it (auto — no user confirmation needed)

**Do not ask** "Should I add tests?" — when any of the above apply, generate the test file as part of the implementation.

## Prerequisites

- Read `agent-os/rules/fe-testing-requirements.mdc` for the naming/location conventions
...
See `agent-os/skills/fe-route-island/SKILL.md` and `docs/reference/route-island-structure.md`.
...
**Route islands:** hooks/components/forms tests live in those directories; page test at island root. See `agent-os/skills/fe-route-island/SKILL.md`.

**Full rules, E2E workflow, and route inventory:** read `agent-os/skills/fe-e2e-testids/SKILL.md` and `docs/reference/e2e-testids-inventory.md`.
...
1. Ensure testids exist — invoke **fe-e2e-testids** skill; match `docs/reference/e2e-testids-inventory.md`
2. Run **`pnpm validate:testids`** after adding pages/forms/shell surfaces
3. Follow **fe-playwright-e2e** skill — hybrid selectors via `tests/utils/e2e-hybrid.ts`

**Hybrid pattern** (`agent-os/skills/fe-playwright-e2e/SKILL.md`):
